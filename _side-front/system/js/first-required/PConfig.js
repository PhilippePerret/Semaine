...
REQUIS
------
  - le module 'MiniAide.js' et 'system/css/MiniAide.css'
  - system/css/PConfig.css
  - Le fichier `app/js/first-required/PConfig.js` définissant les données
    propres à l'application
  - Le `owner` servant à l'instanciation doit définir la propriété `configPath`,
    qui retourne le chemin d'accès au fichier de configuration (un fichier .json).
    Par exemple, si le fichier se trouve enregistré de façon générale, comme
    un fichier de préférence générale pour l'application, le propriétaire peut
    le définir par :
      get configPath(){
        return this._configpath || (this._configpath = path.join(app.getPath('userData'),'config.json'))
      }
  - un appel à <owner>.config.edit() pour modifier les configurations.
...
DATA_MINI_AIDE
  Permet de définir les aides qui s'afficheront quand on cliquera sur le
  bouton "?" en regard de chaque configuration.
  Cf. le fichier app/js/first-required/PConfig.js
